...
class Animal(Organism):
    ...
    def choose_new_location(self):
        ...
        if new_direction == self.get_map().directions_num:
            ...
            new_location = self._location.get_neighbour_location(new_direction, self.get_speed())
    ...
    def mate(self):
        new_location = self.get_map().get_first_free_neighbour(self._location)

        if new_location is not None:
            self.tlog(" mates")
            return self._world.add_organism(self.spawn_offspring(new_location))
        else:
            return None
    ...
    def __str__(self):
        return "Animal at " + str(self._location)
