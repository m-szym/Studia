...
class CyberSheep(Animal):
    ...
    def spawn_offspring(self, new_location):
        return CyberSheep(self._world, new_location)

    def __str__(self):
        return "Cyber Sheep at " + str(self._location)

    def choose_new_location(self):
        p = self.get_map().bfs(self._location,
                               lambda imap, loc: imap[loc] is not None and imap[loc].get_type() == ORGANISM_TYPES["Hogweed"])

        if p is None:
            print(f"Didn't find hogweed, wandering from {self._location}")
