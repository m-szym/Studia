...
float pixels_wall[] = { //tekstura o wymiarach 2x2; dla kazdego punktu okreslone skladowe RGB koloru 
	1.0f, 1.0f, 1.0f,   0.1f, 0.7f, 0.1f,	1.0f, 1.0f, 1.0f,
	0.5f, 0.1f, 0.1f,   1.0f, 1.0f, 1.0f,	0.1f, 0.1f, 0.5f,
	1.0f, 1.0f, 1.0f,   0.1f, 0.1f, 0.1f,	1.0f, 1.0f, 1.0f,
};
...
void configure_texture_wall1()
{
	...
	glTexImage2D(GL_TEXTURE_2D, 0, GL_RGB, 3, 2, 0, GL_RGB, GL_FLOAT, pixels_wall); // ladowanie do tekstury tablicy pikseli
}
...
bool wallCollision(glm::vec3 pos) {
	if (pos.x >= -5.0f && pos.x <= 5.0f && pos.z <= -4.0f && pos.z >= -6.0f)
		return true;
	if (pos.x >=  0.0f && pos.x <= 5.0f && pos.z <=  1.0f && pos.z >= -1.0f)
		return true;
	if (pos.x >= -2.0f && pos.x <= 2.0f && pos.z <= 7.0f && pos.z >= -1.0f)
		return true;

	return false;
}

void redraw_triangle() {
	glBindVertexArray(vao[1]);
...
int main(int argc, char ** argv)
{
	...
	glm::vec3 position = glm::vec3(0.0f, 1.0f, -2.0f); //poczatkowe polozenie kamery
	glm::vec3 direction = glm::vec3(0.0f, 0.0f, -1.0f); //poczatkowy kierunek, w ktorym kamera jest skierowana

	glm::vec3 step = glm::vec3(0.25f, 0.0f, 0.25f);
	float angle = 0.0f;
	float angleStep = M_PI / 60;
	...
	while (true)
	{
		if (SDL_PollEvent(&windowEvent))
		{
			...
			if (windowEvent.type == SDL_KEYDOWN)
			{
				...
				case SDLK_UP:
					if (!wallCollision(position + (step * direction)))
