...
% y_approximation2 - tablica komórkowa przechowująca wartości nmax funkcji aproksymujących dane roczne.
%   - nmax = length(y_yearly)-1
%   - y_approximation2{i} stanowi aproksymację stopnia i
%   - y_approximation2{i} stanowi wartości funkcji aproksymującej w punktach x_fine
...
if isfield(energy, country) && isfield(energy.(country), source)
    ...
    for i = 1:length(y_yearly)
        X = dct2_custom(y_yearly, i);
        y_approximation2{i} = idct2_custom(X, i, N, P);
        mse(i) = sum((idct2_custom(X, i, N, length(x_coarse)) - y_yearly) .^ 2) / length(y_yearly);
    end
    
    for i = 1:length(y_yearly)-1
        msek(i) = sum((y_approximation2{i} - y_approximation2{i+1}) .^ 2) / length(x_fine);
    end
    ...
    for i = 1:length(degrees)
        X = dct2_custom(y_yearly, degrees(i));
        plot(x_fine, idct2_custom(X, degrees(i), N, P), 'DisplayName', string(degrees(i)));
    end
    ...
    size(y_approximation2)
    size(mse)
    size(msek)
    y_approximation = {};
    for i = 1:9
       y_approximation{i} = y_approximation2{i}; 
    end
    y_approximation = y_approximation2;
    mse = mse.';
    msek = msek.';
